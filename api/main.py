...
async def create_transcode_task(
    ...
    try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                try:
                    ...
                    if response.status_code >= 400:
                        raise HTTPException(400, f"Media URL not accessible: HTTP {response.status_code}")
        ...
        try:
            ...
            logger.info(f"=== PUBLISHING START: task {task_id} with {len(transcode_config.profiles)} profiles ===")
            ...
            if published_count == 0:
                await TaskCRUD.update_task_status(db, task_id, TaskStatus.FAILED, f"Failed to publish any transcode messages: {failed_profiles}")
            ...
            try:
                if 'task' in locals():
                    await TaskCRUD.update_task_status(db, task_id, TaskStatus.FAILED, f"Unexpected error: {str(e)}")
            except:
                pass
            # Clean up uploaded file if exists
            if source_key:
        ...
        return {
            ...
            "face_detection_enabled": bool(transcode_config.face_detection_config and transcode_config.face_detection_config.enabled),
...
async def get_task_status(
    ...
    def calculate_progress(task):
        ...
        completion_percentage = (processed_count / expected_count * 100) if expected_count > 0 else 0
    ...
    def format_profile_config(profile):
        ...
        if not isinstance(profile, dict):
            return {"id": str(profile), "display_name": str(profile), "config_summary": "Basic profile"}
        ...
        if output_type == 'video':
            if profile.get('video_config'):
                ...
                if vc.get('max_width') or vc.get('max_height'):
                    config_summary.append(f"Max: {vc.get('max_width', 'auto')}x{vc.get('max_height', 'auto')}")
                if vc.get('bitrate'):
            ...
            if profile.get('image_config'):
                ...
                if ic.get('max_width') or ic.get('max_height'):
                    config_summary.append(f"Max: {ic.get('max_width', 'auto')}x{ic.get('max_height', 'auto')}")
            ...
            if profile.get('gif_config'):
                ...
                if gc.get('width') or gc.get('height'):
                    config_summary.append(f"Size: {gc.get('width', 'auto')}x{gc.get('height', 'auto')}")
                if gc.get('duration'):
    ...
    return {
        ...
        "config": task.config,
        "profiles": [format_profile_config(p) for p in task.config.get('profiles', [])] if task.config else [],
...
async def get_tasks_summary(
    ...
    result = await db.execute(
        select(
            ...
            func.count(TranscodeTaskDB.task_id).label('count')
        ).group_by(TranscodeTaskDB.status)
    )
    ...
    total_result = await db.execute(
        select(func.count(TranscodeTaskDB.task_id))
    )
...
async def delete_task(
                ...
                for item in item_list:
                    ...
                    try:
                        ...
                        if settings.aws_endpoint_public_url in url:
                            key = url.replace(f"{settings.aws_endpoint_public_url}/{settings.aws_bucket_name}/", "")
                        else:
                ...
                if face.get("avatar_url"):
                    try:
                        ...
                        if settings.aws_endpoint_public_url in avatar_url:
                            key = avatar_url.replace(f"{settings.aws_endpoint_public_url}/{settings.aws_bucket_name}/", "")
                        else:
                ...
                if face.get("face_image_url"):
                    try:
                        ...
                        if settings.aws_endpoint_public_url in face_image_url:
                            key = face_image_url.replace(f"{settings.aws_endpoint_public_url}/{settings.aws_bucket_name}/", "")
                        else:
    ...
    if delete_files:
        logger.info(f"Task {task_id} deleted successfully. Deleted {len(deleted_files)} files, {len(failed_deletions)} failed")
        return {
...
async def retry_task(
    ...
    if delete_files:
        ...
        logger.info(f"Retrying task {task_id} - preserving S3 files, only clearing database records")
    ...
    try:
        ...
        if config.face_detection_config and config.face_detection_config.enabled:
            try:
                ...
                face_message_id = pubsub_service.publish_face_detection_task(face_message)
                face_detection_published = True
                logger.info(f"✅ RETRY: Published face detection task, message_id: {face_message_id}")
        ...
        if published_count == 0:
            ...
            await TaskCRUD.update_task_status(
                ...
                task_id,
                TaskStatus.FAILED,
                error_message="Failed to publish any retry messages"
            )
            return {
