...
from video_to_webp_converter import VideoToWebPConverter
...
class WebPTestSuite:
    def __init__(self, input_video: str, output_dir: str = "webp_tests"):
        ...
        self.converter = VideoToWebPConverter()
        self.results = []
    ...
    def run_preset_tests(self) -> List[Dict[str, Any]]:
        ...
        for preset in presets:
            ...
            if result.get('success'):
                print(f"✅ {name}: {result.get('file_size_mb', 0):.2f}MB, {result.get('conversion_time_seconds', 0):.1f}s")
            else:
                print(f"❌ {name}: FAILED")
    ...
    def run_fps_comparison_tests(self) -> List[Dict[str, Any]]:
        ...
        for fps in fps_values:
            ...
            if result.get('success'):
                print(f"✅ {fps} FPS: {result.get('file_size_mb', 0):.2f}MB, {result.get('frames', 0)} frames")
            else:
    ...
    def run_method_comparison_tests(self) -> List[Dict[str, Any]]:
        ...
        for method in methods:
            ...
            if result.get('success'):
                print(f"✅ Method {method}: {result.get('file_size_mb', 0):.2f}MB, {result.get('conversion_time_seconds', 0):.1f}s")
            else:
