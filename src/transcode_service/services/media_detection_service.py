...
class MediaDetectionService:
    ...
    # Media type mappings
    VIDEO_EXTENSIONS = {'.mp4', '.avi', '.mov', '.mkv', '.webm', '.m4v', '.3gp', '.flv', '.wmv', '.mpg', '.mpeg'}
    ...
    def detect_media_type(
        ...
        Returns:
            'image' or 'video'
        """
        logger.debug(f"Detecting media type for filename={filename}, content_type={content_type}, url={url}")
    ...
    def filter_profiles_by_input_type(
        ...
        for profile in profiles:
            ...
            if profile_input_type is None:
                ...
                logger.info(f"Skipping profile {profile.id_profile}: input_type={profile_input_type}, media_type={media_type}")
        
        logger.info(f"Filtered {len(filtered_profiles)} profiles for {media_type} input, skipped {len(skipped_profiles)} profiles")
