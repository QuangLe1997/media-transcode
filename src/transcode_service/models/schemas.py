...
class GifConfig(BaseModel):
    ...
    width: Optional[int] = Field(
        default=None, ge=32, le=1920, description="Width in pixels (auto if None)"
    )
    height: Optional[int] = Field(
        default=None, ge=32, le=1080, description="Height in pixels (auto if None)"
    )
    duration: Optional[float] = Field(
        default=None, gt=0, description="Duration in seconds (full video if None)"
    )
...
class WebPConfig(BaseModel):
    ...
    start_time: float = Field(default=0, ge=0, description="Start time in seconds")
    quality: int = Field(
        default=80, ge=0, le=100, description="WebP quality (0=lossy, 100=lossless)"
    )
    lossless: bool = Field(default=False, description="Use lossless WebP compression")
    animated: bool = Field(
        default=True, description="Create animated WebP (false for single frame)"
    )
    method: int = Field(
        default=4, ge=0, le=6, description="WebP compression method (0=fast, 6=slowest)"
    )
    loop: int = Field(default=0, description="Loop count (0 = infinite)")
...
class TranscodeProfile(BaseModel):
    ...
    ffmpeg_args: Optional[List[str]] = Field(
        default=None, description="FFmpeg arguments for video/image output"
    )
    video_config: Optional[VideoConfig] = Field(
        default=None, description="Video configuration (alternative to ffmpeg_args)"
    )
    image_config: Optional[ImageConfig] = Field(
        default=None, description="Image configuration (alternative to ffmpeg_args)"
    )
    gif_config: Optional[GifConfig] = Field(default=None, description="GIF-specific configuration")
    webp_config: Optional[WebPConfig] = Field(
        default=None, description="WebP-specific configuration"
    )
    ...
    def validate_profile_configs(self):
        ...
        if self.output_type == OutputType.VIDEO:
            ...
            if self.ffmpeg_args:
                raise ValueError(
                    "ffmpeg_args should not be used with GIF output, use gif_config instead"
                )
            ...
            if self.ffmpeg_args:
                raise ValueError(
                    "ffmpeg_args should not be used with WEBP output, use webp_config instead"
                )
            if self.video_config or self.image_config or self.gif_config:
                raise ValueError(
                    "video_config, image_config, and gif_config cannot be used with WEBP output"
                )
...
class TranscodeConfig(BaseModel):
    ...
    face_detection_config: Optional[FaceDetectionConfig] = Field(
