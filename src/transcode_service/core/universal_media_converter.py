...
class UniversalMediaConverter:
    ...
    def _get_output_info(self, output_path: str, conversion_time: float, input_type: str, output_format: str,
    ...
    def _print_result_summary(self, result: Dict[str, Any]):
        ...
        print("\n" + "=" * 80)
        print(
            f"🎯 {result.get('input_type', 'MEDIA').upper()} → {result.get('output_format', 'FORMAT').upper()} CONVERSION RESULTS")
        print("=" * 80)
...
Examples:
  ...
  python universal_media_converter.py input.avi output.mp4 --codec h264 --crf 20
    ...
    parser.add_argument("--preset", choices=["default", "photo", "picture", "drawing", "icon", "text"],
                        default="default")
