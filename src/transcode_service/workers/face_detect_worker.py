...
try:
    handlers.append(logging.handlers.RotatingFileHandler('logs/face_detect_consumer.log', maxBytes=5 * 1024 * 1024, backupCount=3))
...
class FaceDetectionWorker:
    ...
    def _ensure_models_ready(self):
        ...
        try:
            ...
            if models_ready:
                ...
                logger.warning("⚠️  Some models may not be available. Worker will continue with limited functionality.")
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                import cv2
                health_status["dependencies"]["opencv"] = {"available": True, "version": cv2.__version__}
            except ImportError:
                health_status["dependencies"]["opencv"] = {"available": False, "version": None}
                
            try:
                import onnxruntime
                health_status["dependencies"]["onnxruntime"] = {"available": True, "version": onnxruntime.__version__}
            except ImportError:
                health_status["dependencies"]["onnxruntime"] = {"available": False, "version": None}
            
            try:
                import numpy as np
                health_status["dependencies"]["numpy"] = {"available": True, "version": np.__version__}
            except ImportError:
            ...
            models_healthy = all(model["available"] and model["valid"] for model in health_status["models"].values())
    ...
    def process_task(self, message: FaceDetectionMessage):
        ...
        try:
            ...
            s3_config = message.config.get("s3_output_config", {
            ...
            output_data = self._upload_face_avatars(detection_result, message.task_id, s3_config, task_temp_dir)
    ...
    def _process_video(self, video_path: str, config: dict, task_temp_dir: str = None) -> dict:
        ...
        try:
            ...
            processor_config = {
                ...
                "face_landmarker_score_threshold": config.get("face_landmarker_score_threshold", 0.85),
    ...
    def _upload_face_avatars(self, detection_result: dict, task_id: str, s3_config: dict = None, task_temp_dir: str = None) -> Dict[str, List[str]]:
        ...
        try:
            ...
            for filename in os.listdir(faces_dir):
                if filename.endswith('.jpg'):
                    ...
                    s3_url = s3_service.upload_file_from_path(local_path, s3_key, skip_base_folder=True)
    ...
    if health_status['status'] == 'unhealthy':
        ...
        for model_name, model_info in health_status['models'].items():
            if not model_info['available'] or not model_info['valid']:
                logger.warning(f"Model {model_name}: available={model_info['available']}, valid={model_info['valid']}")
